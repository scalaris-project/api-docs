...
The following set of calls are used to communicate and interact with blockchains remotely over the Scalaris network via XRouter. 
...
```shell
scalaris-cli xrGetNetworkServices
```
...
score           | int     | The node's score based on quality of service. A score of `-200` will ban the node for a 24hr period. You can change the ban threshold with the `xrouterbanscore` setting in `scalaris.infonf`. See [node scoring](#xrouter-node-scoring) for more details.
...
```shell
scalaris-cli xrGetBlockCount SYS 2
```
...
nodepubkey*     | string  | The node ID.
score*          | int     | The respective node's score based on quality of service. A score of `-200` will ban the node for a 24hr period. You can change the ban threshold with the `xrouterbanscore` setting in `scalaris.infonf`. See [node scoring](#xrouter-node-scoring) for more details.
...
```shell
scalaris-cli xrGetBlockHash SYS 482107 2
```
...
```shell
scalaris-cli xrGetBlock SYS 0cf18712db68be85793dc06cd0a4fbc8edb166157e6847bb3c9f55d462b02837 2
```
...
```shell
scalaris-cli xrGetBlocks SYS "0cf18712db68be85793dc06cd0a4fbc8edb166157e6847bb3c9f55d462b02837,52583afcd857e45234e7c8981804b36d13a7b3025c17f3535c3e724542590a79" 2
```
...
```shell
scalaris-cli xrDecodeRawTransaction SYS  2
```
...
```shell
scalaris-cli xrGetTransaction SYS 9e5db236f75babe4e28c17f0ed1eddbcfdb5bde8a69750e1a4952d110c620e51 2
```
...
```shell
scalaris-cli xrGetTransactions SYS "f7efcb33c817153dbea86b827380ffce108c6c8e4707356e874d98d0426339bd,f63543bb90800a601065d6f6d8380d8a98ac1a9f208921febde4eb0168e6fd8e" 2
```
...
```shell
scalaris-cli xrSendTransaction SYS 
```
...
```shell
scalaris-cli xrGetReply 3243a24b-3e9d-40d1-8c33-37a57878ce85
```
...
This call is used to reload `xrouter.conf` and all associated plugin configs after changes have been made to apply newly configured settings without needing to restart the Scalaris client.
...
servicenode     | bool    | Signifies if your client is a Service Node.<br>`true`: Client is a Service Node.<br>`false`: Client is not a Service Node.
config          | string  | The raw text contents of your `xrouter.conf`.
plugins         | object  | An object of each service you have setup and the raw text contents of the respective config file in the `plugins` folder within the Scalaris data directory.
...
<!-- 
  ...
  ```shell
  scalaris-cli xrGetBalance SYS SYb9Gmcwj1aXUV86cKpnCD8SR7hvZgbKTP
  ```
...
<!--  
  ...
  ```shell
  scalaris-cli xrGetBalanceUpdate SYS SYb9Gmcwj1aXUV86cKpnCD8SR7hvZgbKTP 125157
  ```
...
<!-- 
  ...
  ```shell
  scalaris-cli xrGenerateBloomFilter SYb9Gmcwj1aXUV86cKpnCD8SR7hvZgbKTP 
  ```
...
<!-- 
  ...
  ```shell
  scalaris-cli xrGetTransactionsBloomFilter SYS 0000100800
  ```
